...
@Suite("The Proxmox client, against a stubbed transport")
struct ProxmoxAPIClientTests {
    private let url = "https://pve.local:8006"
    private let auth = "PVEAPIToken=user@pve!token=secret"

    private func transport(_ handler: @escaping @Sendable (URLRequest) throws -> StubResponse)
        -> (ProxmoxAPIClient, StubTransport)
    {
    ...
    func buildsRequest() async throws {
        ...
        let (client, transport) = transport { _ in .ok(data) }
    ...
    func toleratesTrailingSlash() async throws {
        ...
        #expect(transport.lastRequest?.url?.path == "/api2/json/cluster/resources")
    }
    ...
    func doesNotRetryUnauthorized() async throws {
        let (client, transport) = transport { _ in .status(401, "authentication failure") }
    ...
    func doesNotRetryServerError() async throws {
        let (client, transport) = transport { _ in .status(500, "boom") }
    ...
    func doesNotRetryGarbage() async throws {
        let (client, transport) = transport { _ in .ok(Data("not json".utf8)) }
    ...
    func retriesNetworkFailures() async throws {
        let (client, transport) = transport { _ in throw URLError(.cannotConnectToHost) }
    ...
    func performsGuestAction() async throws {
        ...
        let (client, transport) = transport { _ in .ok(Data(#"{"data":"\#(upid)"}"#.utf8)) }
    ...
    func keepsTaskIdentifierIntact() async throws {
        let upid = "UPID:pve:00051234:0123ABCD:65F0A1B2:qmstart:100:root@pam:"
        let (client, transport) = transport { _ in
            .ok(Data(#"{"data":{"status":"stopped","exitstatus":"OK"}}"#.utf8))
        }

        try await client.waitForTask(node: "pve", upid: upid, url: url, authHeader: auth)

        let path = try #require(transport.lastRequest?.url?.absoluteString)
